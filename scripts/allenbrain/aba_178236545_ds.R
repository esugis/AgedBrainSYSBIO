# This script combines expression data with tissue annotations in 178236545 data set
# Processed data is a data frame with columns probe_id, sample_num, value, structure_id

# Create the folder where current results will be written
resdir <- "~/AgedBrainSYSBIO/results/allenbrain/178236545_ds"
...
maexp <- read.csv(file = "~/AgedBrainSYSBIO/data/allenbrain/178236545_ds/MicroarrayExpression.csv", header = F)
dim(maexp)
...
probes <- read.csv("/AgedBrainSYSBIO/data/allenbrain/178236545_ds/Probes.csv")
dim(probes)

# Read in sample annotations
sampleann <- read.csv("/AgedBrainSYSBIO/data/allenbrain/178236545_ds/SampleAnnot.csv")
dim(sampleann)
...
save(maexp.probes,file = "maexp.probes_178236545_ds.RData")
write.table(maexp.probes, file = "maexp.probes_178236545_ds.txt", sep = "\t", quote = F, row.names = F)
