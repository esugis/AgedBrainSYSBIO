...
# Dimention after removing duplicates
dim(affy2ensg)

# Save the result
save(affy2ensg, file = "affy2ensg.RData")
...
E_GEOD_28146 <- nc_open("~/absb/data/adn/E-GEOD-28146.nc");
...
E_GEOD_4757<- nc_open("~/absb/data/adn/E-GEOD-4757.nc");
...
save(affy2ensg_all, file = "affy2ensg_all.RData")
write.table(affy2ensg_all, file = "affy2ensg_all.txt", sep = "\t", quote = F, row.names = F)
...
# Save to file
save(selected_affys_ensg, file = "selected_affys_ensg.RData")
write.table(selected_affys_ensg, file = "selected_affys_ensg.txt", sep = "\t", quote = F, row.names = F)

# Unique selected affy probes for the analysis
un_sel_gsub_pr <- unique(selected_affys_ensg$gsub_probes)
length(un_sel_gsub_pr)
save(un_sel_gsub_pr, file = "un_sel_gsub_pr.RData")
